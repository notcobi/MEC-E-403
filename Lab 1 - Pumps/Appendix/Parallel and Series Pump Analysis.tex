...
\begin{table}[h]
    \centering
    \caption{Summary of recorded experimental parallel pump time for 2700 RPM}
    \label{tab:parallel_pump_time}
    \begin{tabular}{p{0.12\textwidth}cccccC{0.15\textwidth}}
    \toprule
    Valve Configuration & \multicolumn{2}{c}{Transducer Output} & \multicolumn{3}{c}{Time to collect water} & Nominal Time, $t$ \\
    & Pump 1 & Pump 2 & Trial 1 & Trial 2 & Trial 3 & \\
    & ($\unit{V}$) & ($\unit{V}$) & ($\unit{s}$) & ($\unit{s}$) & ($\unit{s}$) & ($\unit{s}$) \\
    \midrule
    Fully open & 2.87 & 2.72 & 12.46 & 12.28 & 12.79 & 12.51 \\
    Partial 1 & 3.08 & 2.96 & 14.49 & 14.49 & 14.42 & 14.47 \\
    Partial 2 & 3.52 & 3.53 & 41.05 & 40.99 & 40.99 & 41.01 \\
    Closed & 3.55 & 3.55 & - & - & - & - \\
    \bottomrule
    \end{tabular}
\end{table}
\begin{table}[h]
    \centering 
    \caption{Summary of recorded experimental parallel pump discharge and heads for 2700 RPM}
    \label{tab:parallel_pump_discharge_head}
    \begin{tabular}{p{0.12\textwidth}ccccc}
    \toprule
    Valve Configuration & Volumetric Flow, $Q$ & \multicolumn{3}{c}{Transducer Head, $H_t$} & Corrected Head, $H$ \\
    & & Pump 1 & Pump 2 & Nominal & \\
    & ($\unit{\meter\cubed\per\second}$) & ($\unit{\meter}$) & ($\unit{\meter}$) & ($\unit{\meter}$) & ($\unit{\meter}$) \\
    \midrule
    Fully open & 0.007266 & 10.1 & 9.58 & 9.84 & 11.3 \\
    Partial 1 & 0.006283 & 10.8 & 10.4 & 10.6 & 11.7 \\
    Partial 2 & 0.002217 & 12.4 & 12.4 & 12.4 & 12.5 \\
    Closed & - & 12.5 & 12.5 & 12.5 & 12.5 \\
    \bottomrule
    \end{tabular}
\end{table}
Sample calculations will be shown for the fully open valve configuration for Tables \ref{tab:parallel_pump_time} and \ref{tab:parallel_pump_discharge_head}. The same calculations were done for the other valve configurations. The nominal time was calculated as the average of the three trials by 
\begin{align*}
    t &= \frac{\sum t_i}{n} \\
...
\begin{align*}
    ...
    &= \qty{9.84}{\meter} + \frac{8 \times (\qty{0.007266}{\meter\cubed\per\second})^2}{\pi^2 \times (\qty{0.0381}{\meter})^4 \times \qty{9.81}{\meter\per\second\squared}} \left[1 - \left(\frac{\qty{0.0381}{\meter}}{\qty{0.0508}{\meter}}\right)^4\ \right] \\
    &= \qty{11.3}{\meter}
\end{align*}
...
\begin{table}[h]
    ...
    \begin{tabular}{p{0.12\textwidth}C{0.10\textwidth}C{0.10\textwidth}C{0.10\textwidth}C{0.15\textwidth}C{0.15\textwidth}}
    \toprule
    Config. & Time STDEV, $S_t$ & Time Precision, $P_t$ & Time Bias, $B_t$ & Time Uncertainty, $\delta_t$ & Flow Uncertainty, $\delta_Q$ \\
    & ($\unit{s}$) & ($\unit{s}$) & ($\unit{s}$) & ($\unit{s}$) & ($\unit{\meter\cubed\per\second}$) \\
    \midrule
    Fully open & 0.2587 & 0.6425 & 0.01 & 0.64 & 3.73E-04 \\
    Partial 1 & 0.0404 & 0.1004 & 0.01 & 0.10 & 4.38E-05 \\
    Partial 2 & 0.0346 & 0.0861 & 0.01 & 0.09 & 4.68E-06 \\
    Closed & - & - & - & - & - \\
    \bottomrule
    \end{tabular}
\end{table}
\begin{table}[h]
    ...
    \toprule
    Config. & \multicolumn{2}{c}{Transducer Uncertainty} & \multicolumn{3}{c}{Transducer Head Uncertainty} & Corrected Head Uncertainty, $\delta_H$ \\
    ...
    \midrule
    Fully open & 0.01 & 0.01 & 0.04 & 0.04 & 0.02 & 0.1 \\
    Partial 1 & 0.01 & 0.01 & 0.04 & 0.04 & 0.02 & 0.03 \\
    Partial 2 & 0.01 & 0.01 & 0.04 & 0.04 & 0.02 & 0.02 \\
    Closed & 0.01 & 0.01 & 0.04 & 0.04 & 0.02 & 0.02 \\
    \bottomrule
    \end{tabular}
\end{table}
\noindent Sample calculations are evaluated for the fully open configuration at 2700 RPM. A 95\% confidence interval was used. The standard deviation was calculated by Excel using \texttt{STDEV.S}. The t-distribution value was found using $\alpha/2 = 0.025$ and $\nu = 3 - 1 = 2$. Then,
\begin{align*}
    P_t &= t_{\alpha/2, \nu} \times \frac{S_t}{\sqrt{n}} \\
    &= 4.303 \times \frac{\qty{0.2587}{\second}}{\sqrt{3}} \\
    &= \qty{0.6425}{\second}
\end{align*}
The time bias was approximated to be the resolution of the stopwatch,
\begin{align*}
    B_t &= \qty{0.01}{\second}
\end{align*}
...
\begin{align*}
    ...
    &= \sqrt{(\qty{0.6425}{\second})^2 + (\qty{0.01}{\second})^2} \\
    &= \qty{0.64}{\second}
\end{align*}
The flow uncertainty was calculated by
\begin{align*}
    \delta_Q &= Q \bigg|\frac{\delta_t}{t}\bigg| \\
    &= \qty{0.007266}{\meter\cubed\per\second} \frac{\qty{0.64}{\second}}{\qty{12.51}{\second}} \\
    &= 3.73E-04
\end{align*}
...
\begin{align*}
    ...
    &= \sqrt{\left(\qty{0.02}{\meter}\right)^2 + \left(\frac{2(\qty{11.3}{\meter} - \qty{9.84}{\meter})}{\qty{0.007266}{\meter\cubed\per\second}} \times \qty{3.73E-04}{\meter\cubed\per\second}\right)^2} \\
    &= \qty{0.2}{\meter}
\end{align*}
...
\begin{table}[h]
    \centering
    \caption{Summary of recorded experimental series pump time for 2700 RPM}
    \label{tab:series_pump_time}
    \begin{tabular}{p{0.12\textwidth}cccccC{0.15\textwidth}}
    \toprule
    Valve Configuration & \multicolumn{2}{c}{Transducer Output} & \multicolumn{3}{c}{Time to collect water} & Nominal Time, $t$ \\
    & Pump 1 & Pump 2 & Trial 1 & Trial 2 & Trial 3 & \\
    & ($\unit{V}$) & ($\unit{V}$) & ($\unit{s}$) & ($\unit{s}$) & ($\unit{s}$) & ($\unit{s}$) \\
    \midrule
    Fully open & 0.68 & 1.68 & 16.64 & 16.17 & 16.03 & 16.28 \\
    Partial 1 & 0.9 & 1.84 & 16.71 & 16.58 & 16.76 & 16.68 \\
    Partial 2 & 2.89 & 3.26 & 30.52 & 30.23 & 30.67 & 30.47 \\
    Closed & 3.52 & 3.55 & - & - & - & - \\
    \bottomrule
    \end{tabular}
\end{table}
\begin{table}[h]
    \centering
    \caption{Summary of recorded experimental series pump discharge and heads for 2700 RPM}
    ...
    & ($\unit{\meter\cubed\per\second}$) & ($\unit{\meter}$) & ($\unit{\meter}$) & ($\unit{\meter}$) & ($\unit{\meter}$) \\
    \midrule
    Fully open & 0.005584 & 2.39 & 5.92 & 8.31 & 9.1 \\
    Partial 1 & 0.005449 & 3.17 & 6.48 & 9.65 & 10.4 \\
    Partial 2 & 0.002983 & 10.2 & 11.5 & 21.7 & 21.9 \\
    Closed & - & 12.4 & 12.5 & 24.9 & 24.9 \\
    \bottomrule
    \end{tabular}
\end{table}
\noindent Sample calculations will be shown for the fully open valve configuration. The same calculations were done for the other valve configurations. The nominal time was calculated as the average of the three trials by
\begin{align*}
    t &= \frac{\sum t_i}{n} \\
...
\begin{table}[h]
    ...
    & ($\unit{s}$) & ($\unit{s}$) & ($\unit{s}$) & ($\unit{s}$) & ($\unit{\meter\cubed\per\second}$) \\
    \midrule
    Fully open & 0.3195 & 0.7938 & 0.01 & 0.79 & 2.72E-04 \\
    Partial 1 & 0.0929 & 0.2308 & 0.01 & 0.23 & 7.55E-05 \\
    Partial 2 & 0.2237 & 0.5557 & 0.01 & 0.56 & 5.44E-05 \\
    Closed & - & - & - & - & - \\
    \bottomrule
    \end{tabular}
\end{table}
\begin{table}[h]
    ...
    Partial 1 & 0.01 & 0.01 & 0.04 & 0.04 & 0.05 & 0.05 \\
...
\begin{align*}
    \delta_t &= \sqrt{P_t^2 + B_t^2} \\
    &= \sqrt{(\qty{0.7938}{\second})^2 + (\qty{0.01}{\second})^2} \\
    &= \qty{0.79}{\second}
\end{align*}
The flow uncertainty was calculated by
\begin{align*}
    \delta_Q &= Q \bigg|\frac{\delta_t}{t}\bigg| \\
    &= \qty{0.005584}{\meter\cubed\per\second} \frac{\qty{0.79}{\second}}{\qty{16.28}{\second}} \\
    &= 2.72E-04
\end{align*}
The transducer uncertainty was assumed to be the resolution of the device, $\delta_{V_{\text{tr}}} = \qty{0.01}{\volt}$. Transducer precision error was not considered since calibration was not performed. The transducer heads were found by
\begin{align*}
...
\begin{align*}
    \delta_H &= \sqrt{\left(\delta_{H_t}\right)^2 + \left(\frac{2(H - H_t)}{Q}\delta_Q\right)^2} \\
    &= \sqrt{\left(\qty{0.05}{\meter}\right)^2 + \left(\frac{2(\qty{9.1}{\meter} - \qty{8.31}{\meter})}{\qty{0.005584}{\meter\cubed\per\second}} \times \qty{2.72E-04}{\meter\cubed\per\second}\right)^2} \\
    &= \qty{0.1}{\meter}
\end{align*}
